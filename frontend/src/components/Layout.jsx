...
export default function Layout() {
  ...
  return (
    <div className="min-h-screen flex bg-gray-50 dark:bg-slate-900">
      ...
      {/* Sidebar — FIXED, not scrolling with page */}
      <aside
        className={`fixed inset-y-0 left-0 z-50 w-64 bg-pos-dark text-white transform transition-transform duration-200 ${
      ...
      >
        ...
        {/* Trial Alert — Fixed below logo */}
        {isTrial && trialDaysLeft <= 7 && (
          <div className="shrink-0 mx-4 mt-4 p-3 bg-amber-500/20 border border-amber-500/30 rounded-xl">
        ...
        <div className="shrink-0 border-t border-white/10">
          ...
          <button
            onClick={toggleTheme}
            className="flex items-center gap-3 w-full px-4 py-2.5 text-sm font-medium text-gray-300 hover:bg-white/10 hover:text-white transition-colors"
          >
          ...
          <div className="relative px-4 pb-4 pt-1">
            <button
              ...
              className="flex items-center gap-3 w-full p-2.5 rounded-xl hover:bg-white/10 transition-colors"
            >
              <div className="w-9 h-9 bg-pos-blue rounded-full flex items-center justify-center text-sm font-bold shrink-0">
            ...
            {userMenuOpen && (
              <div className="absolute bottom-full left-4 right-4 mb-2 rounded-xl shadow-lg border overflow-hidden bg-white dark:bg-slate-800 border-gray-100 dark:border-slate-700">
                <button
                  onClick={() => {
                    logout();
                    navigate("/login");
